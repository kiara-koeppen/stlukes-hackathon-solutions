...
- **When:** Wednesday Aug 19, 2026 (full day) + Thursday Aug 20, 2026 (half day).
...
- **Environment:** non-CSP sandbox workspace (Azure, West US 2), AD group `R-AiDevCollab_GG_AP`.
  Genie Code + Model Serving/FMAPI enabled. **Comms = email** (no Teams guest access).
...
Databricks and not Copilot/Fabric*: governed Genie over real UC tables (lineage, row/column
security), Agent Bricks, constraint optimization Copilot-in-Excel couldn't do (#6), MLflow eval for
"how do you know it's right," and the clean POC to prod path. That last one matters because Molly's
...
**Morning: Databricks-led demos, anchored on HTM (#14, non-PHI, safe to project).**
- Walk the room through the HTM blueprint live: ingest to medallion, Metric Views, then the money
  shot: a **Genie Space** answering "What anesthesia machines need replacement in 2026?" over
  governed tables. This is the demo that lands the Genie-vs-Copilot story.
- Show the feature palette they'll draw from (AI functions in SQL, Agent Bricks, Vector Search,
  Model Serving, Databricks Apps, MLflow eval). Keep it concrete, tie each to a use case.
...
diversion, the Genie space for HTM).
...
- **#12 Diversion (G1):** rules before ML. The synthetic data has planted diversion patterns, so they
  ...
  the most sensitive.
- **#14 HTM (G3):** Genie Space is the star. Get the gold asset table + Metric Views right and the NL
  queries just work. Easiest to make demo-ready, good confidence builder.

## Open items to close on the prep call (proposed Wed 7/29 PM MT)
