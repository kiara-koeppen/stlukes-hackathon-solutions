...
```
 ...
 └──────────────────────┘  │        │ available FTE vs required FTE by     │──▶ D ─│ Genie Agent        │
...
*What happens:* a **Genie Agent** over `position_control_gold` + `staffing_forecast_gold` + the Metric View, with curated instructions and sample SQL, so Aireal can ask "which units are short RNs on nights next pay period?" or "show me surplus CNAs in Med-Surg this quarter" in plain language.
*Feature:* **Genie Agent**, embedded via Genie Code.
...
5. A Genie Agent over gold answering 4–5 leader questions, **or** a minimal Databricks App with the net-FTE heatmap. (Pick one surface to finish, not both.)
